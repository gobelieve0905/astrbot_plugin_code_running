...
指南随插件发布在 `guides/batch-api-tasks/`，通过固定只读工具 `code_guide` 加载，不依赖 AstrBot 原生 Skill 的终端读取，因此不会出现在原生「技能」列表。可在插件页面「代码执行任务 → 批量任务指南」查看四个章节。

插件通过 AstrBot 请求钩子提供简短使用指引：仅当当前请求的工具列表已包含本插件的 `code_start` 和 `code_guide`，且工具和插件均开启时生效。不修改用户人格、不添加被筛选掉的工具、不限定任何平台或项目，适用于安装本插件并授权相应工具的用户。无需手动复制人格提示；模型仍需按任务选择是否使用指南。
