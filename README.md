...
`code_api` 通过通用本地协议连接 API 插件：query/offset 搜索已启用工具，tool 查看完整定义，附 arguments/constraints 预检 Schema 与固定约束。不发业务请求、不授予权限；平台规则和实时权限在执行时重新校验。缺少兼容 API 插件时明确报错，纯计算继续独立工作。

后台「代码执行任务 → 执行环境手册」和 code_guide 提供环境总览、API 契约、程序执行、文件与恢复四个章节。请求钩子仅在当前工具列表已允许使用本插件代码工具时提供能力说明，不改人格、不添加被筛选掉的工具、不要求特定任务流程。

每轮程序自动保存为 submitted_code.py，受同一会话用户文件隔离保护，不允许覆盖。可通过 previous_files 复用保存的代码、响应和检查点后修改程序继续；保留期限和文件额度仍适用，不自动恢复程序栈、不无限续跑、不重放写请求。已有输入如需复用需显式保存到输出。
